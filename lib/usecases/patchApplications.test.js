  ...
  const databaseSpy = jest.fn(() => {
    ...
    for (let count = 1; count <= numberOfApplicationsInResponse; count++) {
      databaseResponse.push({
        ...
        business_address_first_line: `addrline1${count}`,
        business_address_second_line: `addrline2${count}`,
        business_address_third_line: `addrline3${count}`,
        business_address_fourth_line: '',
        business_address_postcode: `Post Code${count}`,
...
describe('patchApplications', () => {
  ...
  test('inserts the author into the query for the notes', async () => {
    ...
    await patchApplications(container)({
      author: 'foo',
      grant_type: 'lrsg_closed_businesses',
    });
  ...
  test('exports correct grant and amount for grant_type', async () => {
    ...
    const { csvString } = await patchApplications(container)({
      author: 'foo',
      grant_type: 'lrsg_sector',
    });

    expect(csvString).toEqual(expect.stringContaining(',AmountToPay1,'));
  });

  test('SQL query includes type of grant', async () => {
    const { databaseSpy, container } = createContainerAndDatabaseSpy(10);
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining('lrsg_closed_businesses_amount'),
      {
        user: expect.anything(),
      }
    );
  ...
  test('throws an exception with invalid grant_type', async () => {
    const { container } = createContainerAndDatabaseSpy(10);
    const res = patchApplications(container)({
      author: 'foo',
      grant_type: 'fake_grant',
    });
    expect(res).rejects.toThrow('Invalid grant type');
  });
});
