...
const applicationSchema = yup.object().shape({
  eligibilityCriteria: yup
    ...
    .shape({
      ...
      businessSizeId: yup
        ...
        .required(),
      typeOfBusinessId: yup.string().oneOf(options.TYPE_OF_BUSINESS).required(),
  ...
  contact: yup
    .object()
    .shape({
      contactTypeId: yup.string().oneOf(options.CONTACT_TYPE).required(),
  ...
  business: yup
    ...
    .shape({
      ...
      percentageFallInIncome: yup.number().min(0).max(100).required(),
      rateableValue: yup
        .number()
        .integer()
        .nullable()
  ...
  turnover: yup
    .object()
    .shape({
      turnover: yup.number().integer().required(),
      year1819: yup.number().integer().required(),
      year1920: yup.number().integer().required(),
    })
    .required(),
  fixedPropertyCosts: yup
    .object()
    .shape({
      year2018To2019: yup.number().integer().required(),
      year2019To2020: yup.number().integer().required(),
  ...
  supplementaryInformation: yup
    .object()
    .shape({
      businessAccounts: yup.array().of(yup.string().required()).required(),
      fixedPropertyCosts: yup.array().of(yup.string().required()).required(),
      fallInIncome: yup.array().of(yup.string().required()).required(),
      identity: yup.array().of(yup.string().required()).required(),
  ...
  declaration: yup
    .object()
    .shape({
      stateAidOptionId: yup.string().oneOf(options.STATE_AID_OPTION).required(),
      dateOfAid: yup.string().nullable(),
      organisationProvidingAid: yup.string().nullable(),
      ...
      permittedToAcceptStateAidGrant: yup
        ...
        .transform(stringToBool)
        .oneOf([true]),
      readUnderstoodDeclaration: yup.bool().oneOf([true]),
    })
    .required(),
});
