...
export const updateApplication = ({ getDbInstance }) => async ({
  ...
  } else if (data.restartGrantAmount) {
    return processRestartGrantAmount(
      addHistoryEntry,
      updateRestartGrantAmount,
      data.restartGrantAmount
    );
  } else {
  ...
  async function processRestartGrantAmount(
    recordHistoryEntry,
    update,
    grantAmountAwarded
  ) {
    if (!RESTART_GRANT_AMOUNT.includes(grantAmountAwarded)) {
      return createResponse(INVALID_GRANT_AMOUNT);
    }

    await updateRestartGrantAmount(
      dbInstance,
      grantAmountAwarded,
      clientGeneratedId
    );
    ...
    await recordHistoryEntry(
      ...
      grantApplicationId,
      user,
      `Restart Grant awarded amount updated to £${grantAmountAwarded}`
    );

    return createResponse(null);
  }

  async function updateRestartGrantAmount(
    db,
    grantAmountAwarded,
    clientGeneratedId
  ) {
    const updateGrantAmountQuery = `
      UPDATE application_assessment
      SET restart_grant_amount = $(grantAmountAwarded)
      FROM grant_application
      WHERE application_assessment.grant_application_id = grant_application.id
      AND grant_application.client_generated_id = $(clientGeneratedId);`;
    await db.none(updateGrantAmountQuery, {
