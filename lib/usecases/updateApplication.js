import { APPLICATION_STATE, LRSG_GRANT_AMOUNT } from 'lib/dbMapping';
...
export const updateApplication = async ({ clientGeneratedId, data, user }) => {
  ...
  if (data.status) {
    ...
    return processGrantAmountAwarded(data.lrsgClosedBusinessesAmount);
  } else {
  ...
  async function processGrantAmountAwarded(grantAmountAwarded) {
    if (!LRSG_GRANT_AMOUNT.includes(grantAmountAwarded)) {
